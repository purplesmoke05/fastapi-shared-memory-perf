...
docker run --name normal-vegeta --rm --network normal_default -i peterevans/vegeta sh -c "echo 'GET http://fastapi-py310-normal:5000/' | vegeta attack -rate=200 -duration=10s | tee results.bin | vegeta report"
echo "=========================================="
echo "Run fastapi-py310-shared"
echo "=========================================="
echo "Run vegeta to Shared(200req/sec, 10sec)"
docker run --name shared-vegeta --rm --network shared_default -i peterevans/vegeta sh -c "echo 'GET http://fastapi-py310-shared:5000/' | vegeta attack -rate=200 -duration=10s | tee results.bin | vegeta report"
