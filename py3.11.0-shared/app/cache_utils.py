...
class DictCache:
    ...
    @staticmethod
    def initialize():
        if os.environ.get("SHARED_MEMORY_USE_LOCK") != "1":
    ...
    def _extend_size_and_write(self, *, key: str = None, value: Any = None, other=(), **kwargs):
        ...
        if mapped_size != shm_cache_size:
            logger.info(f"|thread={threading.get_ident()}| Extend Shared Memory Object Mapping Size for READ. {mapped_size} => {shm_cache_size}")
            ...
            logger.info(f"|thread={threading.get_ident()}| Extend Done for READ")
        ...
        logger.info(f"|thread={threading.get_ident()}| Extend Shared Memory Object Mapping Size for WRITE. {shm_cache_size} => {mod_size}")
        old_shared_memory = self.cache["data"]._memory_block
